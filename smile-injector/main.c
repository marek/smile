...
{	
	...
	HANDLE handle = CreateMutex(NULL, true, "smileloader");
	...
	{
		MessageBox(0, "Could not find dll","Error", MB_ICONERROR);
		return 0;
	}

	MessageBox(0, "Smile Injector\n Press \'END\' to exit without injection ", "Injector", 0);
	...
	if(!ForceLibrary(dllname, &PI))
	{
		...
		MessageBox(0, "Could not inject dll", "Error", MB_ICONERROR);
	}
