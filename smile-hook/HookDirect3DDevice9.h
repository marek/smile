...
#include <d3d9.h>
class HookDirect3DDevice9 : public IDirect3DDevice9
{
public:
	// We need d3d so that we'd use a pointer to HookDirect3D9 instead of the original IDirect3D9 implementor
	// in functions like GetDirect3D9
	HookDirect3DDevice9(IDirect3D9* d3d, IDirect3DDevice9* device) : m_d3d(d3d), m_device(device)
	{
	}
